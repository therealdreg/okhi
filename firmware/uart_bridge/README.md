...
`uart_bridge.uf2` is a release input. `make_release.py` builds this project and copies the `.uf2` out of `build/`. Build output is not versioned, so a release always compiles it first.
